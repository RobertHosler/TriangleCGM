...
function queryBGD(unitsHint) {
  ...
  .then(function (response) {
      ...
      .then(function(data) {
        ...
        let diffMs =date.getTime() - data[0].date // milliseconds between update & now              
        if(isNaN(diffMs)) {
           console.log('Not a number, set to -1 to indicate unknown')
           diffMs = -1
        } else {
        ...
        // Send the data to the device in reverse (oldest first)
