...
function setDate() { 
  ...
  let prettyDate = day + ", " + month + " " + date;
  // document.getElementById("date").text = shortDate;
  document.getElementById("date").text = prettyDate;
}
...
function addSecond() {
  ...
  document.getElementById("minutes").text = Math.round(totalSeconds / 60000 * 10)/10 + ' min';
}
  ...
  // Next pull does not get caculated right
   if(data.age < 0) {
    ...
    if (data.delta && data.delta > 0) {
      deltaText = "+";//add plus sign if delta is positive
    }
    var units = '';
    if (data.units_hint === "mgdl") {
      units = "mg/dL";
    } else {
      units = data.units_hint;
    }
    document.getElementById("delta").text = deltaText + data.delta + ' ' + units
    totalSeconds = data.age;
    setStatusImage('checked.png')   
  }
  clearTimeout(timeOut);
  var updateTime = 60000 * 2;//1 minute
  timeOut = setTimeout(fiveMinUpdater, 300000) // 5 minutes
}
